...
void i2c_start_it(I2C_TypeDef* I2Cx, i2c_events_vars_t* i2c_event_vars,
                  uint8_t addr, uint8_t data_count, uint8_t data_buffer[])
{
    if( (I2Cx->SR2 & I2C_SR2_BUSY) == 0 )
    {
        i2c_event_vars->i2c_addr = addr;
        i2c_event_vars->i2c_data_count = data_count;
        i2c_event_vars->i2c_buffer_ptr = data_buffer;
        I2Cx->CR1 |= I2C_CR1_START;
    }
}
